...
| NRLMSISE-00 implementation (`drag_*_const`) | 26 m ISS, 293 m at 300 km, 13 m GTO over 3 days = 0.7–2 × 10⁻⁴ of the drag-only displacement | fixed-index density along the ISS orbit agrees to +0.01 % mean / 0.06 % rms (GMAT `AtmosDensity` vs satkit at the same lat/lon/alt/time); the residual is the integrated effect of that plus integration noise |
