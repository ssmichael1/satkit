...
Usage::
    ...
    python tests/gmat/generate.py --gmat "~/Projects/GMAT R2026A" --spk ~/de440.bsp \
        --sw-file ~/SW-All.txt       # CelesTrak file, needed by the drag_*_sw cases
...
def drag_lines(drag, sw_file) -> list:
    """GMAT ForceModel lines for the drag block (field names per the GMAT
    R2026A User Guide, ForceModel -> Fields)."""
    if drag is None:
        return ["FM.Drag = None;"]
    lines = [
    ...
    if drag["weather"] == "constant":
        lines += [
            ...
            "FM.Drag.MagneticIndex = %r;" % drag["gmat_kp"],  # Kp, not Ap
        ]
...
def build_script(case: dict, spk: Path, report_path: Path, sw_file=None) -> str:
    ...
    # GMAT wants one assignment per line.
    lines = [
...
def sw_file_updated(sw_file) -> str:
    """The 'UPDATED ...' stamp of the CelesTrak file, so the JSON records which
    snapshot GMAT read (observed rows are stable, but be explicit)."""
    if sw_file is None:
        return ""
