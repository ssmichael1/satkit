...
**Solid Earth tides.** GMAT's `Solid` tide model includes the IERS 2010 §6.2.2 Step-2 frequency-dependent corrections; satkit's `SolidStep1` implements Step 1 only (see [Force Model](forces.md#solid-earth-tides)). Under `full` this adds 0.4–0.7 m at LEO over 7 days and about 2 m on Molniya, whose perigee passes sample the tidal field most strongly.

**Relativity.** GMAT's relativistic correction includes the geodesic (de Sitter) precession and Lense–Thirring terms in addition to the Schwarzschild term satkit models (see [Force Model](forces.md#general-relativistic-correction-schwarzschild)). Under `gr` this adds about 0.2 m at LEO and 1 m at 200,000 km over 7 days.
