...
impl GaussJackson8 {
    ...
    pub fn integrate<T: FloatScalar, const D: usize>(
        ...
        while (tf - t_current) * dir > zero {
            ...
            if (remaining.abs() - h.abs()).to_f64().unwrap_or(0.0)
                < -1.0e-12 * h.abs().to_f64().unwrap_or(1.0)
            {
            ...
            for _corr_iter in 0..settings.max_corrector_iters {
                ...
                if dr / rmag < settings.corrector_tol && dv / vmag < settings.corrector_tol {
                    break;
                }
...
mod tests {
    ...
    fn gj8_damped_oscillator() {
        ...
        let expected = env * (f64::cos(wd * t_final) + 0.5 * gamma / wd * f64::sin(wd * t_final));
        assert!(
            (sol.r[0] - expected).abs() < 1e-8,
            "damped r = {:.6e}, expected {:.6e}, err = {:.3e}",
            sol.r[0],
            expected,
            (sol.r[0] - expected).abs()
        );
    }
    ...
    fn gj8_kepler_circular_10_orbits() {
        ...
        assert!(
            pos_err < 1e-6,
            "position drift after 10 orbits = {:.3e}",
            pos_err
        );
    }
    ...
    fn gj8_dense_interpolation_harmonic() {
        ...
        let sol = GaussJackson8::integrate(0.0, tf, &r0, &v0, |_t, r, _v| -*r, &settings).unwrap();
