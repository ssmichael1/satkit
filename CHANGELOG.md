...
- **Breaking:** `satproperties()` takes keyword arguments only (`cdaoverm=`, `craoverm=`, `thrusts=`, `ecom=`) and a positional call raises `TypeError`: positional arguments were bound as `(craoverm, cdaoverm)`, the reverse of the documented `(cdaoverm, craoverm)`, so drag and radiation pressure were silently swapped for anyone who followed the docs — check old scripts that passed them positionally ([#222](https://github.com/ssmichael1/satkit/pull/222))
...
- **Behaviour change:** UTC before 1972 follows the "rubber second" model of USNO `tai-utc.dat` / ERFA `dat` from 1961-01-01 (TAI − UTC was 0 before 1972; it stays 0 before 1961), so pre-1972 UTC labels convert to TAI/TT/TDB/GPS correctly (up to 9.9 s different) and match ERFA to 1 µs; the 86,410 s 1971-12-31 (`23:59:60`–`69`) becomes the real 0.107758 s step, `time.UNIX_EPOCH` is the UTC label (raw 8,000,082 µs), UT1 − UTC is interpolated as UT1 − TAI (UT1 continuous across every step), Gregorian and Unix-time seconds round to the microsecond, and a stored pre-1972 instant (e.g. a pickle) now prints a label up to ~10 s different ([#223](https://github.com/ssmichael1/satkit/pull/223))
...
- **Wrong results:** `quaternion * V` for an Nx3 array applied the *inverse* rotation in 0.14.1–0.23.1 (a column-major matrix read as row-major; single 3-vectors were correct), so results computed that way should be recomputed; new matrix-layout tests cover every binding that passes a matrix. Frames and bindings: the Earth rotation angle is evaluated from the two-part date as in ERFA `era00` (0.31 mas → 9 µas against ERFA, 1 cm at LEO / 6 cm at GEO in every full ITRF ↔ GCRF rotation), and the approximate TEME → GCRF (`qteme2gcrf`, `rotation_approx` / `transform_state_approx` to GCRF, EME2000, ICRF, TLE fitting) no longer carries a stray 0.3–0.6″ polar-motion rotation (0.8″ → 0.52″ max against the full reduction); a one-element list or array of times returns a one-element list / array (not a scalar), as the stubs document; quaternion rotation accepts integer arrays and lists; `duration / int` works (the stub's unimplemented `duration + float` is removed); explicit `None` is accepted for optional keywords (`satproperties`, `satstate.propagate`, `propagate`, `nrlmsise00`); `satstate` accepts `datetime` times; `TLE.from_url` / `omm_from_url` honour offline mode; `gravity` / `gravity_and_partials` reject misspelt keywords with `TypeError`; the enum types pickle; docs: the GMST formula, what `qteme2gcrf`, `qteme2itrf` and `qmod2gcrf` compute, and the accuracy of `eqeq`, `gast` and `qtod2mod_approx` ([#222](https://github.com/ssmichael1/satkit/pull/222))
