...
- **Breaking:** the keyword arguments of `duration()`, `propsettings()`, `satproperties()`, `itrfcoord()`, `sgp4()`, `propagate()`, `satstate.propagate()`, `gravity()`, `gravity_and_partials()` and `nrlmsise00()` are real keyword-only parameters (visible to `inspect.signature` and stubtest); an unknown keyword raises `TypeError` (was `ValueError`), every call that worked before gives the same result ([#236](https://github.com/ssmichael1/satkit/pull/236))
...
- **Breaking:** Python `TLE.from_lines()` / `from_file()` / `from_url()` always return a `list[TLE]`, even for one element set (use `TLE.from_lines(lines)[0]`; no TLEs still raises `ValueError`); a record that fails to parse names its input line and satellite, and flags a line longer than 69 characters; new Rust `TLE::records()` iterator yields each record separately (skip bad ones with `.filter_map(Result::ok)`), with optional checksum validation (`.check_checksums(true)`, Python `check_checksum=True` on `from_lines` / `from_file`) ([#240](https://github.com/ssmichael1/satkit/pull/240))
...
- EOP and data layer: a duplicated or unsorted `finals2000A.all` no longer panics (and poisons every later EOP call) but loads with duplicate dates de-duplicated (observed row, else the later one) and LOD filled after sorting; `require_eop_coverage` also refuses spans before 1973-01-02; stale IERS predictions warn once; the offline EOP refresh error says the copy is kept and lists the IERS mirrors; zero-EOP error corrected to ~12″ (hundreds of metres at LEO); future-stamped refresh markers are ignored; one failed feed no longer stops `update_datafiles()` from finishing and reloading the rest; **Breaking:** the optional `satkit-data` bundle / `satkit[data]` extra is dropped and no longer searched — provision with `update_datafiles()` and `SATKIT_DATA` ([#242](https://github.com/ssmichael1/satkit/pull/242))
- Time parsing and calendar: `from_rfc3339` / `from_string` no longer drop or misread UTC offsets (`±HH:MM`, `±HHMM`, `±HH` are applied; trailing input and malformed or out-of-range `%z` offsets are errors), fractions beyond 6 digits round instead of truncating, `day_of_week` is exact to the last microsecond, dates before −4712 are correct, and years outside 0000–9999 format as ISO 8601 `±YYYY` and parse back ([#243](https://github.com/ssmichael1/satkit/pull/243))
- Python bindings: `sgp4` is thread-safe on a shared TLE (it could panic with `PanicException: Already mutably borrowed`); **shape change:** a one-element TLE list keeps its axis (`sgp4([tle], t)` is `(1, 3)`, was `(3,)`) and empty lists give empty arrays; stubs match runtime (`rotation` lists, `propresult.interp`, `time - [times]`, `gravity` array-likes, integer batch state transforms); `geodetic` / `propstats` pickle; NaN/inf day counts, a missing `propagate` end time, `quaternion * scalar` and `TLE.epoch = [list]` raise clear `ValueError` / `TypeError` ([#245](https://github.com/ssmichael1/satkit/pull/245))
