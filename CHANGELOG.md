...
- **Breaking:** Python `TLE.from_lines()` / `from_file()` / `from_url()` always return a `list[TLE]`, even for one element set (use `TLE.from_lines(lines)[0]`; no TLEs still raises `ValueError`); a record that fails to parse names its input line and satellite, and flags a line longer than 69 characters; new Rust `TLE::records()` iterator yields each record separately (skip bad ones with `.filter_map(Result::ok)`), with optional checksum validation (`.check_checksums(true)`, Python `check_checksum=True` on `from_lines` / `from_file`) ([#238](https://github.com/ssmichael1/satkit/pull/238))
