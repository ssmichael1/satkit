...
- CI data cache: `download_data.py --all-eop-sources` keeps IERS `finals2000A.all` current and fetches CelesTrak `EOP-All.csv` only when missing (for its 1962–1972 rows, never re-requested), and the cache key now includes the download script, so a change to it regenerates the cache ([#224](https://github.com/ssmichael1/satkit/pull/224))
