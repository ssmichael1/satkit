...
- **Behaviour change:** UTC before 1972 follows the "rubber second" model of USNO `tai-utc.dat` / ERFA `dat` from 1961-01-01 (TAI − UTC was taken as 0 before 1972), so pre-1972 UTC labels convert to TAI/TT/TDB/GPS correctly (up to 9.9 s different) and match ERFA to 1 µs; the 86,410 s 1971-12-31 (`23:59:60`–`69`) becomes the real 0.107758 s step, `time.UNIX_EPOCH` is the UTC label (raw 8,000,082 µs), UT1 − UTC is interpolated as UT1 − TAI, and a stored pre-1972 instant (e.g. a pickle) now prints a label up to ~10 s different ([#PRNUM](https://github.com/ssmichael1/satkit/pull/PRNUM))
